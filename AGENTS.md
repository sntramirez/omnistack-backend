...
- Cuando un servicio requiere código de autorización homologado (`AD_SERVICIO_PARAMETROS.ID_HOMOLOGADO = 'S'`), no manipular el authorization directamente en las estrategias — la homologación se resuelve en `TransactionOrchestrationService` de forma transparente para la estrategia. Para capabilities post-PRECHECK (EXECUTE, VERIFY, REVERSE, CREATE_TICKET), el orquestador resuelve automáticamente el authorization original del proveedor a partir del código homologado que envía el POS, antes de invocar la estrategia. Las estrategias y adapters siempre reciben el authorization original y no deben contener lógica de resolución inversa.
