...
- ECUABET CASH_OUT (`rms_item_code=100708846`) expone en `input_fields` solo `withdrawId` y `password` para `PRECHECK`; los campos de reverso no forman parte del contrato de catalogo.
- BET 593 CASH_OUT (`rms_item_code=100708848`) expone en `input_fields` solo `document` y `withdrawId` para `EXECUTE`; los campos de validacion y reverso no forman parte del contrato de catalogo.
